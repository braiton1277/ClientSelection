...
#
# NOVO (pedido do Marcelo):
# - probing_loss NÃO fixo: usa offset aleatório de batches por round e por cliente,
#   evitando sempre avaliar os mesmos batches (mesmo com shuffle=False).
#
# =================================================================================
...
def probing_loss_random_offset(
    ...
    """
    Probing não-fixo:
    - escolhe um offset aleatório (em batches) e mede 'batches' batches a partir dali.
    - funciona mesmo com shuffle=False (não precisa mexer no DataLoader).
    """
    model.eval()
    if rng is None:
...
def compute_deltas_proj_mom_probe_now_and_fo(
    ...
    mom_beta: float = 0.90,
    round_seed: int = 0,  # <<< NOVO: seed do round p/ probing não-fixo
    ...
    # probing não-fixo: offset aleatório por cliente (derivado do round_seed)
    for i, (tr_loader, ev_loader) in enumerate(zip(client_train_loaders, client_eval_loaders)):
        ...
        probe_now.append(float(probing_loss_random_offset(model, ev_loader, batches=probe_batches, rng=rng_i)))
