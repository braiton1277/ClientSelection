# fl_vdn_cifar10_pool50_k15_RANDOM_vs_VDN_DRIFT_RESET_TARGETED.py
# =================================================================================
#  MESMO script que você colou, mas com **ATAQUE DIRECIONADO (targeted label flip)**:
#   - Em vez de trocar o rótulo para "um aleatório qualquer", usamos um mapeamento fixo
#     (ex: cat->dog, dog->cat, etc).
#   - Mantém as propriedades que você queria:
#       * determinístico por amostra (pré-computa flipped_label[i] com seed fixa)
#       * switchable (enabled / attack_rate por cliente em tempo de execução)
#       * attack_rate controla fração de amostras atacadas (0..1)
#
# =================================================================================
        ...
        for i, idx in enumerate(self.indices):
            ...
            if y in self.target_map:
                ...
                if self.only_map_classes:
                    # se não está no mapa, não altera
                    y_new = y
                else:
                    # fallback: escolhe um rótulo aleatório diferente (mantém compatibilidade)
                    y_new = rng.randint(0, self.n_classes - 1)
                    if y_new >= y:
                        y_new += 1

            # segurança: garante dentro do range
            y_new = int(np.clip(y_new, 0, self.n_classes - 1))

            # se por acaso mapear para ele mesmo, força um "vizinho" (raríssimo)
            if y_new == y:
...
# ============================
# Compute deltas + prox + probe_now (+ gref)
# ============================
...
# ============================
# VDN features: x_i em R^5 + bias => d=6
# ============================
def build_base_features(
    ...
    prox = prox.astype(np.float32)
    prox_pos = np.maximum(0.0, prox).astype(np.float32)
    prox_neg = np.maximum(0.0, -prox).astype(np.float32)
    ...
    X = np.stack([prox_pos, prox_neg, pz, sn, tn], axis=1).astype(np.float32)
    return X
...
def build_context_matrix_vdn(
    ...
    x = build_base_features(prox, probe_ema, staleness, streak)  # (N,5)
    bias = np.ones((x.shape[0], 1), dtype=np.float32)
    return np.concatenate([bias, x], axis=1).astype(np.float32)  # (N,6)


# ============================
# Graph sketch + neighbors/weights (só p/ drift detection)
# ============================
class DeltaSketcher:
    def __init__(self, P: int = 2048, seed: int = 0):
        self.P = int(P)
        self.seed = int(seed)
        self.idx: Optional[np.ndarray] = None
        self.idx_t: Optional[torch.Tensor] = None
    ...
    def ensure(self, dim: int):
        ...
        P = min(self.P, dim)
        self.idx = rng.choice(dim, size=P, replace=False).astype(np.int64)
        self.idx_t = torch.tensor(self.idx, dtype=torch.long, device=DEVICE)

    @torch.no_grad()
    def sketch_normed(self, deltas: List[torch.Tensor]) -> np.ndarray:
        assert len(deltas) > 0
        dim = int(deltas[0].numel())
        self.ensure(dim)

        xs = []
        for dw in deltas:
            v = dw.view(-1).index_select(0, self.idx_t)  # (P,)
            xs.append(v.float())
        X = torch.stack(xs, dim=0)  # (N,P)
        X = X / (X.norm(dim=1, keepdim=True) + 1e-12)
        return X.detach().cpu().numpy().astype(np.float32)


def graph_neighbors_and_weights(
    deltas: List[torch.Tensor],
    sketcher: DeltaSketcher,
    top_m: int = 5,
    tau: float = 0.5,
) -> Tuple[List[List[int]], List[np.ndarray]]:
    N = len(deltas)
    if N <= 1 or top_m <= 0:
        return [[] for _ in range(N)], [np.zeros((0,), dtype=np.float32) for _ in range(N)]

    S = sketcher.sketch_normed(deltas)  # (N,P)
    sim = (S @ S.T).astype(np.float32)  # (N,N)
    ...
    for i in range(N):
        nbr_idx = np.argpartition(-sim[i], M)[:M]
        nbr_idx = nbr_idx[np.argsort(-sim[i, nbr_idx])]
        z = sim[i, nbr_idx] / max(1e-6, float(tau))
        z = z - z.max()
        a = np.exp(z).astype(np.float32)
        a = a / (a.sum() + 1e-12)
        neighbors.append(nbr_idx.tolist())
        weights.append(a.astype(np.float32))
...
def normalized_entropy(p: np.ndarray) -> float:
    ...
    Hmax = float(np.log(p.size + 1e-12))
    return float(H / max(1e-12, Hmax))


class GraphDriftDetector:
    """
    drift_flag = 1 quando (hit >= patience).
    hit = z_turnover>=th_turnover OR z_entropy>=th_entropy
    """
    def __init__(self, window: int = 40, patience: int = 3, z_turnover_th: float = 2.0, z_entropy_th: float = 2.0):
        self.window = int(window)
        self.patience = int(patience)
        self.z_turnover_th = float(z_turnover_th)
        self.z_entropy_th = float(z_entropy_th)

        self.turnover_hist = deque(maxlen=self.window)
        self.entropy_hist = deque(maxlen=self.window)

        self.prev_neighbors: Optional[List[Set[int]]] = None
        self._consec = 0

    @staticmethod
    def _z(x: float, hist: deque) -> float:
        arr = np.asarray(list(hist), dtype=np.float64)
        if arr.size < 8:
            return 0.0
        m = float(arr.mean())
    ...
    def reset(self):
        self.turnover_hist.clear()
        self.entropy_hist.clear()
        self.prev_neighbors = None
        self._consec = 0

    def update(self, neighbors: List[List[int]], weights: List[np.ndarray]) -> Tuple[int, Dict[str, float]]:
        cur_sets = [set(nbr) for nbr in neighbors]

        if self.prev_neighbors is None:
            med_j = 1.0
        else:
            js = [jaccard(cur_sets[i], self.prev_neighbors[i]) for i in range(len(cur_sets))]
            med_j = float(np.median(js))

        turnover = float(1.0 - med_j)

        ent = [normalized_entropy(w) for w in weights]
        ent_med = float(np.median(ent)) if len(ent) > 0 else 0.0

        self.turnover_hist.append(turnover)
        self.entropy_hist.append(ent_med)

        z_turn = self._z(turnover, self.turnover_hist)
        z_ent = self._z(ent_med, self.entropy_hist)

        hit = bool((z_turn >= self.z_turnover_th) or (z_ent >= self.z_entropy_th))
        if hit:
            self._consec += 1
        else:
            self._consec = 0

        flag = 1 if (self._consec >= self.patience) else 0
        self.prev_neighbors = cur_sets

        info = {
            "turnover": float(turnover),
            "entropy_med": float(ent_med),
            "z_turnover": float(z_turn),
            "z_entropy": float(z_ent),
            "hit": 1.0 if hit else 0.0,
            "consec": float(self._consec),
        }
        return flag, info
...
class VDNSelector:
    ...
    def reset_replay_buffer(self):
        self.buf = PrioritizedReplayJoint(
            capacity=self.buf_size, n_agents=self.n_agents, d_in=self.d_in,
            alpha=self.per_alpha, eps=self.per_eps, seed=int(self.seed) + 12345
        )
        self.total_updates = 0
        self.total_samples_drawn = 0
        self._train_calls = 0
...
def run_experiment(
    ...
    targeted_only_map_classes: bool = True,  # se True, só flip nas classes que estão no map
    target_map: Optional[Dict[int, int]] = None,
    ...
    # RESET behavior
    reset_warmup_rounds: int = 50,
    reset_on_two_drift_flags: bool = True,
    ...
    drift_z_turnover: float = 2.0,
    drift_z_entropy: float = 2.0,
    ...
    out_path = Path(out_dir) / f"results_random_vs_vdn_driftreset_targeted_seed{SEED}_{run_id}.json"
    ...
    log = {
        "meta": {
            ...
            "reset_warmup_rounds": int(reset_warmup_rounds),
            "reset_on_two_drift_flags": bool(reset_on_two_drift_flags),
            ...
            "graph_top_m": int(graph_top_m),
            "graph_tau": float(graph_tau),
            "sketch_P": int(sketch_P),

            "graph_drift_detector": {
                "window": int(drift_window),
                "patience": int(drift_patience),
                "z_turnover_th": float(drift_z_turnover),
                "z_entropy_th": float(drift_z_entropy),
            },
        },
        "attack_schedule": [],
        "tracks": {
    ...
    def save_json_safely():
        for key in ["random", "vdn_noreset", "vdn_reset"]:
    ...
    def maybe_open_phase_on_drift(track_key: str, drift_flag: int, prev_flag: int, round_t: int, attacked_snapshot: List[int]):
        if drift_flag == 1 and prev_flag == 0:
            phases = log["tracks"][track_key]["selection_phases"]
            if len(phases) > 0 and phases[-1].get("end_round", None) is None:
                phases[-1]["end_round"] = int(round_t - 1)
            start_new_phase(track_key, round_t, attacked_snapshot)
    ...
    for cid, idxs in enumerate(client_idxs):
        ...
        client_train_loaders.append(
            DataLoader(ds_c, batch_size=64, shuffle=True, generator=g_train, worker_init_fn=seed_worker, num_workers=0)
        )
        client_eval_loaders.append(
            DataLoader(ds_c, batch_size=64, shuffle=False, num_workers=0)
        )
    ...
    # VDN SHARED (antes do primeiro reset)
    model_vdn_shared = copy.deepcopy(base).to(DEVICE)
    staleness_sh = np.zeros(n_clients, dtype=np.float32)
    streak_sh = np.zeros(n_clients, dtype=np.int32)
    probe_ema_sh = np.zeros(n_clients, dtype=np.float32)
    loss_hist_sh: List[float] = []
    pending_sh: Optional[Tuple[np.ndarray, np.ndarray, float]] = None

    # drift (shared)
    sketcher_det = DeltaSketcher(P=sketch_P, seed=SEED + 3001)
    drift_det_sh = GraphDriftDetector(
        window=drift_window, patience=drift_patience,
        z_turnover_th=drift_z_turnover, z_entropy_th=drift_z_entropy
    )
    prev_drift_flag_sh = 0

    # agente shared
    agent_sh = VDNSelector(
        n_agents=n_clients,
        d_in=6,
        k_select=k_select,
        hidden=marl_hidden,
    ...
    start_new_phase("vdn_noreset", 1, sorted(list(attacked_set)))
    start_new_phase("vdn_reset", 1, sorted(list(attacked_set)))
    ...
    agent_rs = None
    drift_det_rs = None
    prev_drift_flag_rs = 0
    rs_extra_warmup_left = 0
    ...
    print(f"Tracks: [RANDOM] vs [VDN drift NO reset] vs [VDN drift RESET]\n")
    ...
    try:
        for t in range(1, rounds + 1):
            ...
            deltas_r, _, _, _ = compute_deltas_prox_probe_now(
                model_rand,
            ...
            # ============================================================
            # TRACKS B/C: VDN (shared até reset)
            # ============================================================
            def vdn_step(
                model: nn.Module,
                agent: VDNSelector,
                ...
                pending: Optional[Tuple[np.ndarray, np.ndarray, float]],
                drift_det: GraphDriftDetector,
                prev_drift_flag: int,
                force_random_override: bool = False,
                allow_train: bool = True,
            ):
                acc = eval_acc(model, test_loader, max_batches=80)
                _l_before = eval_loss(model, val_loader, max_batches=eval_max_batches)

                deltas, prox, probe_now, _ = compute_deltas_prox_probe_now(
                    model,
                    client_train_loaders,
                    client_eval_loaders,
                    val_loader,
                ...
                update_ema(probe_ema, probe_now, alpha=ema_alpha, init_if_zero=True)

                nbr, w = graph_neighbors_and_weights(deltas=deltas, sketcher=sketcher_det, top_m=graph_top_m, tau=graph_tau)
                drift_flag, _info = drift_det.update(nbr, w)

                obs = build_context_matrix_vdn(prox=prox, probe_ema=probe_ema, staleness=staleness, streak=streak)

                if pending is not None:
                    o_prev, a_prev, r_prev = pending
                    agent.add_transition(obs=o_prev, act=a_prev, r=r_prev, obs2=obs, done=False)

                force_rand = force_random_override or (agent.buf.n < warmup_transitions)

                act, sel = agent.select_topk_actions(
                    obs=obs,
                    eps=marl_eps,
                    swap_m=marl_swap_m,
                ...
                apply_fedavg(model, deltas, sel)
                update_staleness_streak(staleness, streak, sel)

                l_after = eval_loss(model, val_loader, max_batches=eval_max_batches)
                loss_hist.append(l_after)
                r = windowed_reward(loss_hist[:-1], l_after, W=reward_window_W)
                pending = (obs.copy(), act.copy(), float(r))
                ...
                if allow_train:
                    can_train = (
                        (t >= start_train_round)
                        and (t % train_every == 0)
                        and (agent.buf.n >= batch_base)
                    )
                    if can_train and not force_rand:
                        bs = dynamic_batch_size(agent.buf.n, base=batch_base, max_bs=batch_max, ratio=batch_buffer_ratio)
                        _ = agent.train(batch_size=bs, train_steps=updates_per_round)
                        trained = True

                return acc, drift_flag, pending, sel, trained

            if not forked:
                acc_sh, drift_flag_sh, pending_sh, sel_sh, _trained_sh = vdn_step(
                    model=model_vdn_shared,
                    agent=agent_sh,
                    staleness=staleness_sh,
                    streak=streak_sh,
                    probe_ema=probe_ema_sh,
                    loss_hist=loss_hist_sh,
                    pending=pending_sh,
                    drift_det=drift_det_sh,
                    prev_drift_flag=prev_drift_flag_sh,
                    force_random_override=False,
                    allow_train=True,
                )

                log["tracks"]["vdn_noreset"]["test_acc"].append(float(acc_sh))
                log["tracks"]["vdn_reset"]["test_acc"].append(float(acc_sh))
                log["tracks"]["vdn_noreset"]["drift_flag"].append(int(drift_flag_sh))
                log["tracks"]["vdn_reset"]["drift_flag"].append(int(drift_flag_sh))

                bump_counts("vdn_noreset", sel_sh)
                bump_counts("vdn_reset", sel_sh)

                maybe_open_phase_on_drift("vdn_noreset", drift_flag_sh, prev_drift_flag_sh, t, attacked_snapshot)
                maybe_open_phase_on_drift("vdn_reset", drift_flag_sh, prev_drift_flag_sh, t, attacked_snapshot)

                reset_trigger = False
                if reset_on_two_drift_flags:
                    reset_trigger = (drift_flag_sh == 1 and prev_drift_flag_sh == 1)
                else:
                    reset_trigger = (drift_flag_sh == 1 and prev_drift_flag_sh == 0)

                prev_drift_flag_sh = int(drift_flag_sh)

                if reset_trigger:
                    forked = True
                    fork_round = t
                    log_step(f"  >>> RESET TRIGGER @ round {t}: drift_flag==1 por 2 rounds seguidos")

                    model_nr = copy.deepcopy(model_vdn_shared).to(DEVICE)
                    model_rs = copy.deepcopy(model_vdn_shared).to(DEVICE)

                    staleness_nr = staleness_sh.copy()
                    streak_nr = streak_sh.copy()
                    probe_ema_nr = probe_ema_sh.copy()
                    loss_hist_nr = list(loss_hist_sh)
                    pending_nr = pending_sh

                    staleness_rs = staleness_sh.copy()
                    streak_rs = streak_sh.copy()
                    probe_ema_rs = probe_ema_sh.copy()
                    loss_hist_rs = list(loss_hist_sh)
                    pending_rs = None

                    agent_nr = agent_sh

                    agent_rs = VDNSelector(
                        n_agents=n_clients,
                        d_in=6,
                        k_select=k_select,
                        hidden=marl_hidden,
                    ...
                    agent_rs.q.load_state_dict(agent_nr.q.state_dict())
                    agent_rs.q_tgt.load_state_dict(agent_nr.q_tgt.state_dict())
                    agent_rs.opt.load_state_dict(agent_nr.opt.state_dict())
                    agent_rs.reset_replay_buffer()

                    drift_det_nr = drift_det_sh
                    prev_drift_flag_nr = prev_drift_flag_sh

                    drift_det_rs = GraphDriftDetector(
                        window=drift_window, patience=drift_patience,
                        z_turnover_th=drift_z_turnover, z_entropy_th=drift_z_entropy
                    )
                    prev_drift_flag_rs = 0

                    rs_extra_warmup_left = int(reset_warmup_rounds)
                    log["tracks"]["vdn_reset"]["reset_events"].append({"round": int(t)})
                    log["tracks"]["vdn_noreset"]["reset_events"].append({"round": int(t), "did_reset": False})

            else:
                acc_nr, drift_flag_nr, pending_nr, sel_nr, _trained_nr = vdn_step(
                    model=model_nr,
                    agent=agent_nr,
                    staleness=staleness_nr,
                    streak=streak_nr,
                    probe_ema=probe_ema_nr,
                    loss_hist=loss_hist_nr,
                    pending=pending_nr,
                    drift_det=drift_det_nr,
                    prev_drift_flag=prev_drift_flag_nr,
                    force_random_override=False,
                    allow_train=True,
                )
                maybe_open_phase_on_drift("vdn_noreset", drift_flag_nr, prev_drift_flag_nr, t, attacked_snapshot)
                prev_drift_flag_nr = int(drift_flag_nr)

                bump_counts("vdn_noreset", sel_nr)
                log["tracks"]["vdn_noreset"]["test_acc"].append(float(acc_nr))
                log["tracks"]["vdn_noreset"]["drift_flag"].append(int(drift_flag_nr))

                force_rand_rs = (rs_extra_warmup_left > 0)
                allow_train_rs = not force_rand_rs

                acc_rs, drift_flag_rs, pending_rs, sel_rs, _trained_rs = vdn_step(
                    model=model_rs,
                    agent=agent_rs,
                    staleness=staleness_rs,
                    streak=streak_rs,
                    probe_ema=probe_ema_rs,
                    loss_hist=loss_hist_rs,
                    pending=pending_rs,
                    drift_det=drift_det_rs,
                    prev_drift_flag=prev_drift_flag_rs,
                    force_random_override=force_rand_rs,
                    allow_train=allow_train_rs,
                )
                maybe_open_phase_on_drift("vdn_reset", drift_flag_rs, prev_drift_flag_rs, t, attacked_snapshot)
                prev_drift_flag_rs = int(drift_flag_rs)

                bump_counts("vdn_reset", sel_rs)
                log["tracks"]["vdn_reset"]["test_acc"].append(float(acc_rs))
                log["tracks"]["vdn_reset"]["drift_flag"].append(int(drift_flag_rs))

                if rs_extra_warmup_left > 0:
                    rs_extra_warmup_left -= 1

            if t % print_every == 0:
                a_r = log["tracks"]["random"]["test_acc"][-1]
                a_nr = log["tracks"]["vdn_noreset"]["test_acc"][-1]
                a_rs = log["tracks"]["vdn_reset"]["test_acc"][-1]
                d_nr = log["tracks"]["vdn_noreset"]["drift_flag"][-1]
                d_rs = log["tracks"]["vdn_reset"]["drift_flag"][-1]
                msg = (
                    f"[summary @ {t:3d}] "
                    f"RANDOM acc={a_r*100:.2f}% | "
                    f"VDN(noreset) acc={a_nr*100:.2f}% drift={d_nr} | "
                    f"VDN(reset) acc={a_rs*100:.2f}% drift={d_rs} | "
                    f"attacked_total={len(attacked_set)}"
                )
                if forked:
                    msg += f" | fork_round={fork_round} | rs_warmup_left={rs_extra_warmup_left}"
                print(msg, flush=True)

        if not forked:
            if pending_sh is not None:
                o_prev, a_prev, r_prev = pending_sh
                agent_sh.add_transition(obs=o_prev, act=a_prev, r=r_prev, obs2=o_prev, done=True)
        else:
            if pending_nr is not None:
                o_prev, a_prev, r_prev = pending_nr
                agent_nr.add_transition(obs=o_prev, act=a_prev, r=r_prev, obs2=o_prev, done=True)
            if pending_rs is not None:
                o_prev, a_prev, r_prev = pending_rs
                agent_rs.add_transition(obs=o_prev, act=a_prev, r=r_prev, obs2=o_prev, done=True)
...
if __name__ == "__main__":
    run_experiment(
        ...
        # TARGETED:
        targeted_only_map_classes=True,  # se True, só flip das classes do mapa (aqui todas estão no mapa)
        target_map=None,                # None => usa o default (swaps pareados)
        ...
        reset_warmup_rounds=50,
        reset_on_two_drift_flags=True,
