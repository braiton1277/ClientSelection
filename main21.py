# fl_dqn_per_cifar10.py eh o 
# Federated client selection on CIFAR-10:
#   Track A: RANDOM
#   Track B: DQN (Double DQN + Target Net) + PER
#
# Notes:
# - This script intentionally computes (prox, probe, dloss) for ALL clients each round
#   to build the DQN context (like your MNIST version). That can be heavy on CIFAR-10,
#   so defaults are set to be lighter (local_steps=10, rounds=50, etc.).
#
# Run:
#   python fl_dqn_per_cifar10.py
#
# If using a venv:
#   source marl_env/bin/activate   (Linux/macOS)
#   .\marl_env\Scripts\activate    (Windows PowerShell)
...
# ============================
def make_clients_dirichlet_indices(train_ds, n_clients: int = 20, alpha: float = 0.3, seed: int = 123, n_classes: int = 10) -> List[List[int]]:
...
def run_experiment(
    ...
    client_idxs = make_clients_dirichlet_indices(train_ds, n_clients=n_clients, alpha=dir_alpha, seed=SEED + 777, n_classes=10)
...
if __name__ == "__main__":
    run_experiment(
        ...
        max_per_client=2500,
        local_lr=0.01,
        local_steps=10,       # CIFAR is heavier; 10 is a good start
        probe_batches=1,
        dloss_batches=1,
        ema_alpha=0.10,
        print_every=10,
