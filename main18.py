...
class SmallCNN(nn.Module):
    def __init__(self, n_classes: int = 10):
        super().__init__()
        self.conv1 = nn.Conv2d(3, 32, 3, padding=1)   # 32x32
        self.conv2 = nn.Conv2d(32, 64, 3, padding=1)  # 32x32
        self.pool = nn.MaxPool2d(2)                   # 16x16

        self.conv3 = nn.Conv2d(64, 128, 3, padding=1) # 16x16
        self.conv4 = nn.Conv2d(128, 128, 3, padding=1)# 16x16
        # pool -> 8x8
...
def run_experiment(
    ...
    for t in range(1, rounds + 1):
        ...
        # RANDOM track
        # -------------------------
        log_step("  - RANDOM: deltas + prox + probe_now + dloss_now (todos os clientes)...")
        tb = time.time()
        deltas_r, prox_r, probe_now_r, dloss_now_r = compute_deltas_prox_probe_dloss_now(
            model_rand, client_loaders, val_loader,
            local_lr, local_steps,
        ...
        # BANDIT track
        # -------------------------
        log_step("  - BANDIT: deltas + prox + probe_now + dloss_now (todos os clientes)...")
        tc = time.time()
        deltas_b, prox_b, probe_now_b, dloss_now_b = compute_deltas_prox_probe_dloss_now(
            model_ban, client_loaders, val_loader,
            local_lr, local_steps,
            probe_batches=probe_batches,
            dloss_batches=dloss_batches,
        )
        ...
        # -------------------------
        # METRIC track (optional)
        # -------------------------
        if use_metric_track:
            log_step("  - METRIC: deltas + prox + probe_now + dloss_now (todos os clientes)...")
            td = time.time()
            deltas_m, prox_m, probe_now_m, dloss_now_m = compute_deltas_prox_probe_dloss_now(
                model_met, client_loaders, val_loader,
                local_lr, local_steps,
                probe_batches=probe_batches,
                dloss_batches=dloss_batches,
            )
