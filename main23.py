# fl_dqn_per_cifar10_pool50_k10_reward_window.py
...
# 1) Reward uses a WINDOWED improvement (W=5) + normalization -> less noisy.
# 2) Credit assignment is WEIGHTED per client (not uniform reward/K).
#    We use weights from your existing signals: relu(prox), probe_z, dloss_z.
# 3) Exploration is "swap m clients" inside topK (not full-round random).
#
# Run:
#   python fl_dqn_per_cifar10_pool50_k10_reward_window.py
...
class DQNSelector:
    ...
    def select_topk_with_swaps(
        ...
        """
        Better exploration for large K:
        - pick topK by advantage
        - with prob eps, swap 'swap_m' elements with random non-selected clients
        """
        n = X.shape[0]
        q = self.q_values(X)
        adv = q[:, 1] - q[:, 0]
...
def windowed_reward(loss_history: List[float], new_loss: float, W: int = 5) -> float:
    """
    r = mean(prev W losses) - new_loss, normalized by scale to reduce sensitivity.
    """
...
def weighted_credit_for_selected(
    selected: List[int],
    prox: np.ndarray,
    probe_ema: np.ndarray,
    dloss_ema: np.ndarray,
    ...
    """
    Weights per client based on signals:
      w_i = relu(prox_i) * sigmoid(z_probe_i) * sigmoid(z_dloss_i)
    then normalized to sum 1 among selected.
    """
    sel = np.array(selected, dtype=np.int64)

    relu_prox = np.maximum(0.0, prox).astype(np.float64)

    p = probe_ema.astype(np.float64)
    pm, ps = float(p.mean()), float(p.std() + 1e-6)
    pz = np.clip((p - pm) / ps, -5.0, 5.0)

    d = dloss_ema.astype(np.float64)
    dm, ds = float(d.mean()), float(d.std() + 1e-6)
    dz = np.clip((d - dm) / ds, -5.0, 5.0)

    w = relu_prox * sigmoid01(pz) * sigmoid01(dz)
    w_sel = w[sel]

    # avoid all-zero -> uniform
    if float(w_sel.sum()) <= 1e-12:
        w_sel = np.ones_like(w_sel, dtype=np.float64)

    w_sel = w_sel / (w_sel.sum() + 1e-12)

    out = {int(cid): float(w_sel[i]) for i, cid in enumerate(sel)}
    return out
...
def run_experiment(
    ...
    val_loader = DataLoader(
        ...
        batch_size=256,
        shuffle=val_shuffle,   # <-- you asked about this
        generator=g_dl,
    ...
    # reward histories (store AFTER-update val losses so window makes sense)
    ...
    for t in range(1, rounds + 1):
        ...
        # Select clients with DQN (topK + swaps for exploration)
        ...
        b_reward = windowed_reward(dqn_loss_hist[:-1], l_b_after, W=reward_window_W)

        # ---- Weighted credit among selected
        credit_w = weighted_credit_for_selected(
            selected=selected_b,
            prox=prox_b,
        ...
        for cid in selected_b:
            # per-client reward = (normalized weight) * global reward
            pending_prev.append((cid, 1, Xb[cid].copy(), float(credit_w[int(cid)] * b_reward)))
        for cid in neg_cids:
        ...
        log_step(
            f"  DQN: sel={selected_b} | reward={b_reward:+.5f} (val {l_b_before:.4f}->{l_b_after:.4f}) | "
            f"neg={len(neg_cids)}"
        )
        ...
        if t % print_every == 0:
            print(
                ...
                f"DQN  loss={l_b_after:.4f} r={b_reward:+.4f}",
                flush=True,
            )
...
if __name__ == "__main__":
    run_experiment(
        ...
        print_every=10,
        reward_window_W=5,
        dqn_eps=0.15,         # exploration probability for "swaps"
        dqn_swap_m=2,          # swap 2 members in topK when exploring
        dqn_lr=1e-3,
        dqn_gamma=0.90,
        dqn_train_steps=10,
        dqn_batch_size=256,
        dqn_buf_size=50000,    # bigger buffer helps with 50 clients
