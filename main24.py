# fl_dqn_per_cifar10_pool50_k10_reward_window_loo.py
# Federated client selection on CIFAR-10 (pool=50, K=10):
#   Track A: RANDOM
#   Track B: DQN (Double DQN + Target Net) + PER
#
# FINAL “última saída” antes do MARL:
# 1) Reward mais estável: reward WINDOWED (W=5) + normalizado.
# 2) Crédito por cliente via LOO marginal:
#    credit_i = U(loss_all) - U(loss_leave_one_out_i),
#    onde U(.) é o reward windowed baseado no mesmo histórico.
#
# Run:
#   python fl_dqn_per_cifar10_pool50_k10_reward_window_loo.py
...
class DQNSelector:
    ...
    def select_topk_with_swaps(
        ...
        """
        Exploração melhor quando K é grande:
        - pega topK pelo advantage
        - com prob eps, troca swap_m elementos por aleatórios fora do topK
        """
        n = X.shape[0]
        q = self.q_values(X)
        adv = q[:, 1] - q[:, 0]
        top = np.argsort(adv)[::-1][:k].tolist()
        ...
        if np.random.rand() < eps:
            swap_m = min(swap_m, k)
            top_set = set(top)
            not_sel = [i for i in range(n) if i not in top_set]
            if len(not_sel) > 0:
                out = random.sample(top, swap_m)
                inn = random.sample(not_sel, min(swap_m, len(not_sel)))
                top2 = top.copy()
                for o, i_new in zip(out, inn):
                    top2[top2.index(o)] = i_new
                # garante unicidade
                top2 = list(dict.fromkeys(top2))
                while len(top2) < k:
    ...
    def train(self):
        ...
        for _ in range(self.train_steps):
            ...
            td = (q_sa - y)
            td_abs = td.detach().abs().clamp(min=0.0).cpu().numpy()
...
# ============================
# Reward helpers (windowed + normalized)
# ============================
def windowed_reward(loss_history: List[float], new_loss: float, W: int = 5) -> float:
    """
    U(loss_new) = (mean(prev W losses) - loss_new) / |mean(prev W losses)|
    (normaliza pra reduzir sensibilidade de escala)
    """
...
# ============================
# LOO credit (marginal) for selected clients
# ============================
@torch.no_grad()
def loo_marginal_credits(
    tmp_model: nn.Module,
    w0_flat: torch.Tensor,
    sum_dw: torch.Tensor,
    selected: List[int],
    deltas: List[torch.Tensor],
    val_loader: DataLoader,
    loss_hist_before_round: List[float],
    W: int = 5,
    eval_max_batches: int = 20,
    credit_clip: float = 0.0,      # 0 = no clip; else clip to [-credit_clip, +credit_clip]
    credit_scale: float = 1.0,     # scale credits (useful if values are too tiny)
) -> Dict[int, float]:
    """
    credit_i = U(loss_all) - U(loss_loo_i)
    where:
      loss_all = loss(w0 + mean(dw_S))
      loss_loo_i = loss(w0 + mean(dw_{S\i}))
      U(.) = windowed_reward(hist, loss, W)

    Returns dict {cid: credit}.
    """
    K = len(selected)
    assert K >= 1

    # U_all
    avg_dw = sum_dw / float(K)
    w_all = w0_flat + avg_dw
    load_flat_params_(tmp_model, w_all)
    loss_all = eval_loss(tmp_model, val_loader, max_batches=eval_max_batches)
    U_all = windowed_reward(loss_hist_before_round, loss_all, W=W)

    credits: Dict[int, float] = {}

    if K == 1:
        cid = int(selected[0])
        c = float(credit_scale * U_all)
        if credit_clip > 0:
            c = float(np.clip(c, -credit_clip, credit_clip))
        credits[cid] = c
        return credits
    ...
    for cid in selected:
        cid = int(cid)
        w_loo = w0_flat + (sum_dw - deltas[cid]) / denom
        load_flat_params_(tmp_model, w_loo)
        loss_loo = eval_loss(tmp_model, val_loader, max_batches=eval_max_batches)
        U_loo = windowed_reward(loss_hist_before_round, loss_loo, W=W)

        credit = float(credit_scale * (U_all - U_loo))
        if credit_clip > 0:
            credit = float(np.clip(credit, -credit_clip, credit_clip))
        credits[cid] = credit
...
def run_experiment(
    ...
    val_per_class: int = 200,          # 200 => 2000 samples; aumentar dá reward mais estável
    eval_max_batches: int = 20,        # aumentar reduz ruído (custo ↑)
    # LOO credit config
    loo_credit_clip: float = 0.0,      # 0 = sem clip; ex: 0.02 pode ajudar estabilidade
    loo_credit_scale: float = 1.0,     # ex: 5.0 se o crédito ficar muito pequeno
):
    ...
    # server loaders
    val_idxs = make_server_val_balanced(test_ds, per_class=val_per_class, n_classes=10)
    val_loader = DataLoader(
    ...
    # temp model for LOO evals (avoid cloning every time)
    tmp_model = SmallCNN().to(DEVICE)
    ...
    print(f"Val: per_class={val_per_class} => {10*val_per_class} samples | shuffle={val_shuffle} | eval_max_batches={eval_max_batches}")
    print(f"Reward: window W={reward_window_W} (normalized)")
    print(f"DQN: eps={dqn_eps}, swap_m={dqn_swap_m}, gamma={dqn_gamma}, train_steps={dqn_train_steps}, target_sync_every={dqn_target_sync_every}")
    print(f"PER: alpha={per_alpha}, beta={per_beta_start}->{per_beta_end} over {per_beta_steps} train() calls")
    print(f"Negatives: neg_ratio={neg_ratio} (adds ~neg_ratio*K per round)")
    print(f"Credit: LOO marginal | clip={loo_credit_clip} | scale={loo_credit_scale}")
    print("Comparando: RANDOM vs DQN(Double+PER+LOO)\n")

    # reward histories (store AFTER-update val losses)
    rand_loss_hist: List[float] = []
    dqn_loss_hist: List[float] = []
    ...
    for t in range(1, rounds + 1):
        ...
        # ---- evaluation BEFORE updates (for reporting)
        l_r_before = eval_loss(model_rand, val_loader, max_batches=eval_max_batches)
        a_r = eval_acc(model_rand, test_loader, max_batches=80)

        l_b_before = eval_loss(model_dqn, val_loader, max_batches=eval_max_batches)
        ...
        l_r_after = eval_loss(model_rand, val_loader, max_batches=eval_max_batches)
        rand_loss_hist.append(l_r_after)

        r_reward = windowed_reward(rand_loss_hist[:-1], l_r_after, W=reward_window_W)
        log_step(f"  RANDOM: sel={selected_r} | reward={r_reward:+.5f} (val {l_r_before:.4f}->{l_r_after:.4f})")

        # =========================================================
        # Track B: DQN selector (Double + PER + LOO credit)
        # =========================================================
        ...
        # Select clients with DQN (topK + swaps)
        selected_b = dqn.select_topk_with_swaps(Xb, k=k_select, eps=dqn_eps, swap_m=dqn_swap_m)

        # Save w0 and sum_dw BEFORE updating (needed for LOO)
        w0_flat = flatten_params(model_dqn).clone()
        sum_dw = torch.stack([deltas_b[i] for i in selected_b], dim=0).sum(dim=0)

        # Apply FedAvg update
        apply_fedavg(model_dqn, deltas_b, selected_b)
        update_staleness_streak(staleness_b, streak_b, selected_b)

        # Evaluate after update (this defines the "all-selected" outcome)
        l_b_after = eval_loss(model_dqn, val_loader, max_batches=eval_max_batches)
        dqn_loss_hist.append(l_b_after)

        # Reward uses the SAME history base (exclude current)
        hist_before = dqn_loss_hist[:-1]
        b_reward = windowed_reward(hist_before, l_b_after, W=reward_window_W)

        # ---- LOO marginal credits (K extra evals)
        credits = loo_marginal_credits(
            tmp_model=tmp_model,
            w0_flat=w0_flat,
            sum_dw=sum_dw,
            selected=selected_b,
            ...
            loss_hist_before_round=hist_before,
            W=reward_window_W,
            eval_max_batches=eval_max_batches,
            credit_clip=loo_credit_clip,
            credit_scale=loo_credit_scale,
        )

        # ---- Negative examples (a=0, r=0) from unselected clients
        ...
        # Create pending transitions for THIS round (use LOO credit per selected)
        pending_prev = []
        for cid in selected_b:
            pending_prev.append((int(cid), 1, Xb[cid].copy(), float(credits[int(cid)])))
        for cid in neg_cids:
            pending_prev.append((int(cid), 0, Xb[cid].copy(), 0.0))

        # Train DQN (PER updates priorities internally)
        dqn.train()

        # Logging
        cred_vals = np.array([credits[int(c)] for c in selected_b], dtype=np.float64)
        log_step(
            f"  DQN: sel={selected_b} | reward(all)={b_reward:+.5f} (val {l_b_before:.4f}->{l_b_after:.4f}) | "
            f"LOO credit: mean={cred_vals.mean():+.5f}, min={cred_vals.min():+.5f}, max={cred_vals.max():+.5f} | "
            f"neg={len(neg_cids)}"
        )
        ...
        if t % print_every == 0:
            print(
                f"[summary @ {t:3d}] "
                f"RAND loss={l_r_after:.4f} r={r_reward:+.4f} | "
                f"DQN  loss={l_b_after:.4f} r(all)={b_reward:+.4f} | "
                f"LOO(mean)={cred_vals.mean():+.4f}",
                flush=True,
            )
...
if __name__ == "__main__":
    run_experiment(
        ...
        per_beta_steps=4000,
        val_shuffle=True,
        val_per_class=200,       # aumenta se quiser reward ainda mais estável
        eval_max_batches=20,     # aumenta se quiser (custo ↑)
        loo_credit_clip=0.0,     # ex: 0.02 se quiser segurar outliers
        loo_credit_scale=1.0,    # ex: 5.0 se crédito ficar muito pequeno
    )